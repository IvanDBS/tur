...
const filteredHotels = computed(() => {
  ...
  return props.hotels.filter(hotel => hotel.name.toLowerCase().includes(query))
})
...
const toggleAllHotels = () => {
  if (allHotelsSelected.value) {
    ...
    emit('update:hotels', props.hotels.map(hotel => hotel.id))
  }
}
...
const toggleRegion = (regionId: number) => {
  ...
  if (regionId === 1) {
    ...
    if (currentRegions.includes(1)) {
      ...
      emit('update:regions', props.regions.map(region => region.id))
    }
    return
  }
...
const toggleCategory = (categoryId: number) => {
  ...
  if (categoryId === 1) {
    ...
    if (currentCategories.includes(1)) {
      ...
      emit('update:categories', props.categories.map(category => category.id))
    }
    return
  }
...
const toggleMeal = (mealId: number) => {
  ...
  if (mealId === 1) {
    ...
    if (currentMeals.includes(1)) {
      ...
      emit('update:meals', props.meals.map(meal => meal.id))
    }
    return
  }
...
const toggleOption = (optionId: number) => {
  ...
  if (optionId === 1) {
    ...
    if (currentOptions.includes(1)) {
      ...
      emit('update:options', props.options.map(option => option.id))
    }
    return
  }
